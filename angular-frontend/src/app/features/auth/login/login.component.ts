...
@Component({
  ...
  styles: [`
    ...
    .btn-google {
      width:100%; padding:12px 24px; background:#fff; color:#333;
      border:2px solid #e5e7eb; border-radius:12px; font-size:15px; font-weight:600;
      cursor:pointer; transition:all 0.2s; display:flex; align-items:center; justify-content:center; gap:10px;
    }
    .btn-google:hover { border-color:#ccc; background:#f9fafb; box-shadow:0 2px 8px rgba(0,0,0,0.06); }
...
export class LoginComponent {
  ...
  googleLogin(): void {
    const clientId = (window as any).__env?.googleClientId || '';
    if (!clientId) {
      this.error = 'Google login is not configured yet. Please use email/password login.';
      return;
    }
    // Google Identity Services flow
    const google = (window as any).google;
    if (!google?.accounts?.id) {
      this.error = 'Google Sign-In library not loaded. Please try again.';
      return;
    }
