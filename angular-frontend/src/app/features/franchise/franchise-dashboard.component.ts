...
@Component({
  ...
  template: `
    <div class="dashboard">
      ...
      <div class="greeting-section">
        ...
        <p class="greeting-sub">Here's your franchise performance overview</p>
      </div>
  ...
  styles: [`
    ...
    .greeting-text { font-size: 24px; font-weight: 700; color: var(--gray-900); margin: 0 0 6px; }
    .admin-name {
    ...
    .greeting-sub { font-size: 14px; color: var(--gray-400); margin: 0; }
    ...
    .metric-label { font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: var(--gray-400); margin-top: 2px; }

    /* ========== ROW 2: DISTRIBUTION + RECENT ORDERS (side by side) ========== */
    ...
    .chart-header h3 { font-size: 16px; font-weight: 600; color: var(--gray-900); margin: 0; }
    ...
    .chart-body-table { padding: 0; }

    /* Status Bars */
    .status-bars-list { display: flex; flex-direction: column; gap: 14px; }

    .status-bar-row { display: flex; align-items: center; gap: 12px; }

    .status-bar-label { display: flex; align-items: center; gap: 8px; min-width: 100px; flex-shrink: 0; }

    .status-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }

    .status-name { font-size: 13px; font-weight: 500; color: var(--gray-700); }

    .status-bar-track { flex: 1; height: 10px; background: var(--gray-100); border-radius: 10px; overflow: hidden; }
    ...
    .status-bar-count { font-size: 13px; font-weight: 700; color: var(--gray-900); min-width: 36px; text-align: right; }
    ...
    .status-total strong { font-size: 16px; color: var(--gray-900); }
    ...
    .activity-table th {
      ...
      letter-spacing: 0.5px; color: var(--gray-400); background: var(--gray-100);
    }
    ...
    .activity-table tbody tr { transition: background 0.15s ease; }
    .activity-table tbody tr:hover { background: #fafbfb; }
    .activity-table tr:last-child td { border-bottom: none; }

    .row-num { color: var(--gray-400); font-weight: 500; width: 30px; }
    ...
    .avatar.avatar-sm { width: 30px; height: 30px; font-size: 11px; }

    .user-name { font-size: 13px; font-weight: 600; color: var(--gray-900); }
    .user-email { font-size: 12px; color: var(--gray-400); }

    .order-amount { font-weight: 600; color: var(--gray-900); }

    .status-pill {
      font-size: 11px; font-weight: 600;
      padding: 3px 10px; border-radius: 40px; display: inline-block;
    }

    .status-pill.pending { background: #fff8e1; color: #f59e0b; }
    .status-pill.confirmed { background: #e0f2fe; color: #0284c7; }
    .status-pill.completed { background: #d1fae5; color: #059669; }
    .status-pill.delivered { background: #d1fae5; color: #059669; }
    .status-pill.cancelled { background: var(--red-soft); color: var(--red-primary); }

    /* Empty State */
    .empty-state { padding: 40px 20px; text-align: center; }
    .empty-state svg { margin-bottom: 12px; }
    .empty-state p { color: var(--gray-400); font-size: 13px; margin: 0; }
    ...
    .loading-overlay p { font-size: 13px; color: var(--gray-400); margin: 0; }
    ...
    @media (max-width: 991px) {
      .distribution-orders-grid { grid-template-columns: 1fr; }
    }
...
export class FranchiseDashboardComponent implements OnInit {
  ...
  loadDashboard(): void {
    ...
    this.api.get<any>('/franchise/dashboard').subscribe({
      next: (res) => {
        if (res.success) {
          const d = res.data;
  ...
  getStatusColor(status: number): string {
    const map: Record<number, string> = { 0: '#f59e0b', 1: '#0284c7', 2: '#059669', 3: '#10b981', 4: '#e31b23' };
  ...
  getStatusClass(status: number): string {
    const map: Record<number, string> = { 0: 'pending', 1: 'confirmed', 2: 'completed', 3: 'delivered', 4: 'cancelled' };
