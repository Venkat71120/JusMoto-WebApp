...
class FormBuilderCustom
{
    public static function render_form($id,$form_id = null,$action = null,$button_class = null,$form_class = null){
        ...
        $button_class = 'cmn-btn primary-btn w-100 mt-2';
        ...
        $action = $action ?? route('frontend.contact.message.send');
        $rand = Str::random(10);
        $csrf = '<input type="hidden" name="_token" value="'.csrf_token().'">';
        $google_recaptcha = '<input type="hidden" name="gcaptcha_token" id="gcaptcha_token" value="">';

        return <<<HTML
    <form action="{$action}" {$form_id} method="post" id="custom_form_builder_{$rand}" class="get-in-touch-form custom-form-builder-form message-contact-form {$form_class}" enctype="multipart/form-data">
        {$csrf}
        {$google_recaptcha}
...
HTML;
    ...
    private static function render_submit_button($text,$button_class = '') :string
    {
        ...
        return <<<HTML
            <div class="btn-wrapper send-massage mt-3">
                <button id="contact_form_btn" type="submit" class="{$button_class}">{$text}</button>
                {$preloader}
            </div>
            HTML;
    ...
    private static  function get_field_by_type($type, $name, $placeholder, $options = [], $requried = null, $mimes = null)
    {
        ...
        switch ($type) {
            case('email'):
                ...
                $markup = ' <div class="from-place input-form2 mt-3"> <label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="email" id="' . $name . '" name="' . $name . '" class="form-control input-field" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
                break;
            case('tel'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="input-form input-form2"> <label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="tel" id="' . $name . '" name="' . $name . '" class="form-control input-field" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
                break;
            case('date'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="input-form input-form2"><label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="date" id="' . $name . '" name="' . $name . '" class="form-control input-field" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
                break;
            case('url'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="input-form input-form2"><label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="url" id="' . $name . '" name="' . $name . '" class="input-field" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
                break;
            case('textarea'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="input-form textarea message"><label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <textarea name="' . $name . '" id="' . $name . '" cols="30" rows="5" class="form-control" placeholder="' . __($placeholder) . '" ' . $required_markup . '></textarea></div>';
                break;
            case('file'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $mimes_type_markup = str_replace('mimes:', __('Accept File Type:') . ' ', $mimes);
                $markup = ' <div class="input-form file"> <label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="file" id="' . $name . '" name="' . $name . '" ' . $required_markup . ' class="form-control input-field" > <span class="help-info">' . $mimes_type_markup . '</span></div>';
                break;
            case('checkbox'):
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="checkWrap2 checkbox">  <input type="checkbox" id="' . $name . '" name="' . $name . '" class="form-control" ' . $required_markup . '> <label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label></div>';
                break;
            case('select'):
                $option_markup = '';
                $required_markup = !empty($requried) ? $required_markup_html : '';
                ...
                $markup = ' <div class="select-itms2 select"> <label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <select id="' . $name . '" name="' . $name . '" class="form-control" ' . $required_markup . '>' . $option_markup . '</select></div>';
                break;
            default:
                $required_markup = !empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="from-place input-form2 mt-3"><label for="' . $name . '" class="' . $label_class . '">' . __($placeholder) . '</label> <input type="text" id="' . $name . '" name="' . $name . '" class="form-control input-field" placeholder="' . __($placeholder) . '" ' . $required_markup . '></div>';
                break;
        }
