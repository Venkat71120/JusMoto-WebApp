...
function render_image_markup_by_attachment_id($id, $class = null, $size = 'full')
{
    ...
    if (!empty($image_details)) {
        $img_url=filter_var($image_details['img_url'], FILTER_VALIDATE_URL) ?: '';
        $img_alt = htmlspecialchars($image_details['img_alt'] ?? '', ENT_QUOTES, 'UTF-8');


        if(empty($img_url)){
            $output = '<img src="'.$noImageMarkupRender.'" class="no-image" alt="' . ($img_alt ?? "") . '"/>';
            return $output;
        }
        $output = '<img src="' . $img_url . '" ' . $class_list . ' alt="' . ($img_alt ?? "") . '"/>';
    }else{
...
function render_image_markup_by_attachment_id_for_additional_info_image($id, $class = null, $size = 'full')
{
    if (empty($id)){
        $noImageMarkupRender =  asset('assets/uploads/arrow.png');
        $on_image_output_return = '<img src="'.$noImageMarkupRender.'" class="arrow" alt="arrow"/>';
        return $on_image_output_return;
    }
    ...
    if (!empty($image_details)) {
        ...
        if(empty($img_url)){
            $output = '<img src="'.$noImageMarkupRender.'" class="arrow" alt="' . ($img_alt ?? "") . '"/>';
            return $output;
        }
        $output = '<img src="' . $img_url . '" ' . $class_list . ' alt="' . ($img_alt ?? "") . '"/>';
    }else{

        $output = '<img src="'.$noImageMarkupRender.'" class="arrow" alt="arrow"/>';
    }
...
function render_attachment_preview_for_admin($id)
{
    ...
    $img_url = filter_var($header_bg_img['img_url'], FILTER_VALIDATE_URL) ?: '';
    $img_alt = htmlspecialchars($header_bg_img['img_alt'] ?? '', ENT_QUOTES, 'UTF-8');
...
function render_homepage_meta()
{
    // get home page id from settings
    $home_page_id = get_static_option('home_page');

    // get the home page with metadata
    $homepage = Page::with('meta_data')->find($home_page_id);

    // if no homepage or no meta → fallback to default site meta
    if (!$homepage || !$homepage->meta_data) {
        return render_site_meta();
    }

    $meta = $homepage->meta_data;

    // fallback values
    $default_title = get_static_option('site_title') . ' - ' . get_static_option('site_tag_line');

    $title = $meta->meta_title ?: $default_title;
    $description = $meta->meta_description ?: get_static_option('site_meta_description');
    $keywords = $meta->meta_tags ?: get_static_option('site_meta_tags');

    // Open Graph
    $og_title = $meta->facebook_meta_tags ?: $title;
    $og_description = $meta->facebook_meta_description ?: $description;
    $og_image = $meta->facebook_meta_image
        ? render_og_meta_image_by_attachment_id($meta->facebook_meta_image)
        : render_og_meta_image_by_attachment_id(get_static_option('og_meta_image'));

    // Twitter
    $tw_title = $meta->twitter_meta_tags ?: $title;
    $tw_description = $meta->twitter_meta_description ?: $description;
    $tw_image = $meta->twitter_meta_image
        ? render_twitter_meta_image_by_attachment_id($meta->twitter_meta_image)
        : render_twitter_meta_image_by_attachment_id(get_static_option('og_meta_image'));
...
function render_site_meta_for_general($title){
    ...
    $facebook_meta_tags = $meta_title;
    $facebook_meta_description ='';
    $facebook_meta_image =  "";
    $facebook_meta_image_alt = '';

    $twitter_meta_tags = $meta_title;
...
function render_page_meta_data_for_service($service_details){
    ...
    $user_lang = LanguageHelper::user_lang_slug();
    $site_url = route('frontend.service.details',$service_details->slug);
    ...
    $facebook_meta_image = (is_numeric($service_details->metaData?->facebook_meta_image ?? null)
        ? (get_attachment_image_by_id($service_details->metaData->facebook_meta_image)['img_url'] ?? '')
        : (get_attachment_image_by_id($service_details?->image)['img_url'] ?? ''));

    $facebook_meta_image_alt = (is_numeric($service_details->metaData?->facebook_meta_image ?? null)
        ? (get_attachment_image_by_id($service_details->metaData->facebook_meta_image)['img_url'] ?? '')
        : (get_attachment_image_by_id($service_details?->image)['img_url'] ?? ''));
    ...
    $twitter_meta_image = (is_numeric($service_details->metaData?->twitter_meta_image ?? null)
        ? (get_attachment_image_by_id($service_details->metaData->twitter_meta_image)['img_url'] ?? '')
        : (get_attachment_image_by_id($service_details?->image)['img_url'] ?? ''));
...
if (!function_exists('generateTransactionInvoiceNumber')) {
    function generateTransactionInvoiceNumber()
    {
        // Get the current date in YYYYMMDD format
        $datePrefix = date('Ymd');
        // Find the last invoice number for today
        ...
        // Extract the last sequential number
        $lastNumber = $lastTransaction ? intval(substr($lastTransaction->invoice_number, -4)) : 0;
...
function calculateTaxBasedOnCoordinates($outlet_id,$address,$state_id,$city_id,$latitude,$longitude)
{

    $state_tax_rate = null;

    if(!$address && !$outlet_id)
    {
    ...
    }else
    {
        if($outlet_id)
        {
            $outlet=Admin_outlet_location::where("id",$outlet_id)->first();
            $state_tax = StateTax::where('state_id', $outlet->state_id)->first();

            $city_tax = CityTax::where('city_id', $outlet->city_id)->first();

            if (!empty($city_tax) && $city_tax->tax_rate >= 0) {
                $state_tax_rate = $city_tax->tax_rate;
            } else if(!empty($state_tax) && $state_tax->tax_rate >= 0){
                $state_tax_rate =  $state_tax?->tax_rate ;
            }
        }

        if($state_tax_rate == null && $state_tax_rate == '' && ($state_id != null || $city_id != null))
        {
        ...
        if($state_tax_rate == null && $state_tax_rate == '' && ($latitude != null && $longitude != null))
        {
        ...
        if($state_tax_rate == null && $state_tax_rate == '' && $address != null)
        {
                // Fetch Google Maps API Key
            ...
            $address_geocode_url =  "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=".$address."&key=". $google_map_api_key;

            $address_geocode_data = @file_get_contents($address_geocode_url);

            if ($address_geocode_data == false) {
                $state_tax_rate=null;
            }else{
                $address_geocode_data = json_decode($address_geocode_data, true);

                if($address_geocode_data['predictions'] && $address_geocode_data['predictions'][0]['place_id'])
                {
                    $place_id=$address_geocode_data['predictions'][0]['place_id'];
                    $place_geocode_url = "https://maps.googleapis.com/maps/api/place/details/json?place_id=".$place_id."&key=".$google_map_api_key;
                    $place_geocode_data = @file_get_contents($place_geocode_url);
                    if ($place_geocode_data !== false) {
                        $place_geocode_data = json_decode($place_geocode_data, true);

                        // Check if geocoding was successful
                        if (isset($place_geocode_data['status']) && $place_geocode_data['status'] == 'OK') {
                            $state = $city = null;
                            // Extract state and city from geocode results
                            foreach ($place_geocode_data['result']['address_components'] as $component) {
                                if (in_array('administrative_area_level_1', $component['types'])) {
                                    $state = $component['long_name'];
                                }
                                if (in_array('locality', $component['types'])) {
...
function calculateDeliveryChargeBasedOnCoordinates($outlet_id,$address,$state_id,$city_id,$latitude,$longitude)
{
    $state_delivery_charge_rate = null;
    ...
    {
        if( $state_delivery_charge_rate== null &&  $state_delivery_charge_rate == '' && ($state_id != null || $city_id != null))
        {
            ...
            } else if(!empty($state_delivery_charge) && $state_delivery_charge->delivery_charge >= 0){
                $state_delivery_charge_rate =  $state_delivery_charge?->delivery_charge ;
            }
        }

        if( $state_delivery_charge_rate == null &&  $state_delivery_charge_rate == '' && ($latitude != null && $longitude != null))
        {
                // Fetch Google Maps API Key
            $google_map_api_key = get_static_option('google_map_api_key');
            $geocode_url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" . $latitude . "," . $longitude . "&key=" . $google_map_api_key;
        ...
        if( $state_delivery_charge_rate  == null &&  $state_delivery_charge_rate  == '' && $address != null)
        {
                // Fetch Google Maps API Key
    ...
    return $state_delivery_charge_rate;

}

/**
 * Get video file details by filename
 * @param string $filename
 * @return array
 */
function get_attachment_video_by_filename($filename)
{
    if (empty($filename)) {
        return ['exists' => false];
    }
    ...
    $video_path = dirname(base_path()) . '/assets/uploads/page-builder-videos/' . $filename;
    $video_url = asset('assets/uploads/page-builder-videos/' . $filename);
    ...
    $mime_type = $mime_types[$file_extension] ?? 'video/mp4';
