...
class CategoryManageController extends Controller
{
    ...
    public function categoriesWithService(Request $request){
        ...
        $categories = Category::with('services')->where('status', 1)->where("type",0)
    ...
    public function categoriesWithProduct(Request $request){

        $search = $request->query('name');
        $categories = Category::with('services')->where('status', 1)->where("type",1)
            ->when($search, function ($query, $search) {
                return $query->where('name', 'like', "%{$search}%");
            })
            ->latest()
