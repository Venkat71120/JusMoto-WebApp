...
class SliderController extends Controller
{
    public function add_new_slider(Request $request)
    {
        ...
        if($request->type == 'service'){
            if(!$request->identity_service_id){
                return redirect()->back()->with(FlashMsg::item_delete(__('Please select a service')));
            }
            $identity =  $request->identity_service_id;
            $type =  'service';
        }else if($request->type == 'offer'){
             if(!$request->identity_offer_id){
                return redirect()->back()->with(FlashMsg::item_delete(__('Please select a offer')));
            }
            $identity =  $request->identity_offer_id;
            $type =  'offer';
        }
        else{
            if(!$request->identity_category_id){
                return redirect()->back()->with(FlashMsg::item_delete(__('Please select a category')));
            }
