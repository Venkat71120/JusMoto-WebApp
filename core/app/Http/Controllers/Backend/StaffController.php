...
class StaffController extends Controller
{
    ...
    public function staffSetting(Request $request){
        $field = 'staff_select_settings';
        $value="";
        if($request->$field == 0)
        {
            $value="User can select";
        }
        else if($request->$field == 1)
        {
            $value="User can not select";
        }
        update_static_option($field, $value);
        return redirect()->back()->with(FlashMsg::settings_update());
    }

    public function staffSelectPage()
    {
        $services=Service::all();
        return view("backend.pages.services.admin.staff_setting",compact("services"));
    }
}
