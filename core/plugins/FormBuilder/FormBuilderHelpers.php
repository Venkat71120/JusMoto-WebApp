    ...
    private static function get_field_by_type($type, $name, $placeholder, $options = [], $requried = null, $mimes = null)
    {
        ...
        switch ($type) {
            case 'email':
                ...
                $markup = ' <div class="from-place"> <label for="'.$name.'">'.__($placeholder).'</label> <input type="email" id="'.$name.'" name="'.$name.'" class="form-control" placeholder="'.__($placeholder).'" '.$required_markup.'></div>';
                break;
            case 'tel':
                $required_markup = ! empty($requried) ? $required_markup_html : '';
            ...
            case 'textarea':
                $required_markup = ! empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="message textarea"><label for="'.$name.'">'.__($placeholder).'</label> <textarea name="'.$name.'" id="'.$name.'" cols="30" rows="5" class="form-control" placeholder="'.__($placeholder).'" '.$required_markup.'></textarea></div>';
                break;
            case 'file':
                $required_markup = ! empty($requried) ? $required_markup_html : '';
            ...
            default:
                $required_markup = ! empty($requried) ? $required_markup_html : '';
                $markup = ' <div class="from-place"><label for="'.$name.'">'.__($placeholder).'</label> <input type="text" id="'.$name.'" name="'.$name.'" class="form-control" placeholder="'.__($placeholder).'" '.$required_markup.'></div>';
                break;
        }
