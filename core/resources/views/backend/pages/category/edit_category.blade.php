                ...
                <x-validation.error/>
                 <form action="{{route('admin.category.edit',$category->id)}}" method="post" enctype="multipart/form-data" id="edit_category_form">
                    ...
                    <div class="form__input__flex">
                        ...
                        <div class="form__input__single">
                            <label for="category_type" class="form__input__single__label"> {{__('Category Type')}} </label>
                            <select  name="category_type" id="category_type" class=" form-select category_type">
                                <option value="">{{__('Select Category Type')}}</option>
                                <option value="0" @if($category->type==0) selected @endif>{{__('Service')}}</option>
                                <option value="1" @if($category->type==1) selected @endif>{{__('Product')}}</option>
                                <option value="2" @if($category->type==2) selected @endif>{{__('Both')}}</option>   
                            </select>
                        </div>
                        ...
                        <div class="form__input__single permalink_label">
                            <label class="text-dark form__input__single__label">{{__('Permalink * :')}}
                                ...
                                <span id="slug_edit" class="display-inline">
                                    ...
                                    <input type="text" name="slug" class="form__control radius-5 category_slug mt-2" value="{{$category->slug}}">
                                    <button  class="cmnBtn btn_5 btn_bg_info radius-5 slug_update_button mt-2">{{__('Update')}}</button>
                                </span>
                            </label>
                        </div>
