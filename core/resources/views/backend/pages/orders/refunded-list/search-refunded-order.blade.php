                    ...
                    <tr class="order-row">
                        <td>
                            <span class="order-id-badge">
                                {{ $refundedOrders->firstItem() + $loop->index }}
                            </span>
                        </td>

                        <td>
                            <span class="order-ref">ORD-{{ $data->order_id }}</span>
                        </td>
                        <td>
                            <div class="client-info-wrapper">
                                ...
                                <div class="client-details">
                                    <h6 class="client-name">{{ $data->user?->first_name ?? 'N/A' }}
                                        {{ $data->user?->last_name ?? '' }}</h6>
                                    <span class="client-email">{{ $data->user?->email ?? 'No email' }}</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            <span class="fine-type-badge {{ strtolower($data->fine_type) }}">
                                {{ $data->fine_type }}
                            </span>
                        </td>
                        <td>
                            <span class="fine-amount">{{ float_amount_with_currency_symbol($data->fine_amount) }}</span>
                        </td>
                        <td>
                            <div class="refunded-amount-wrapper">
                                <span class="refunded-amount">{{ float_amount_with_currency_symbol($data->amount) }}</span>
                                @if($data->fine_amount > $data->amount)
                        ...
                        <td>
                            <div class="status-wrapper">
                                <span class="status-badge refunded status-{{ $data->status }}">
                                    @if($data->status == 0)
                                        <i class="las la-clock"></i> {{__('Pending')}}
                                    @elseif($data->status == 1)
                                        <i class="las la-check-circle"></i> {{__('Completed')}}
                                ...
                                <button type="button" class="status-edit-btn refunded_order_status_change_modal"
                                    data-order_id="{{$data->id}}" data-bs-toggle="tooltip" title="{{__('Change Status')}}"
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('admin.refunded-order.details', $data->id) }}" class="action-btn view-btn"
                                    data-bs-toggle="tooltip" title="{{__('View Refund Details')}}">
                                    <i class="las la-eye"></i>
                                </a>
                                <a href="{{ route('admin.main.order.details', $data->order_id) }}"
                                    class="action-btn order-info-btn" data-bs-toggle="tooltip"
                                    title="{{__('View Main Order')}}">
...
<div class="pagination-wrapper mt-4">
    <div class="pagination-info">
        {{ __('Showing') }} {{ $refundedOrders->firstItem() }} {{ __('to') }} {{ $refundedOrders->lastItem() }}
        {{ __('of') }} {{ $refundedOrders->total() }} {{ __('results') }}
    </div>
            ...
            <form action="{{ route('admin.redunded-order.status') }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="status-update-form">
                        <label for="status_id" class="form-label">{{ __('Select New Status') }}</label>
                        <select name="status_id" id="status_id" class="form-select">
                            <option value="0" data-icon="clock">{{ __('Pending') }}</option>
                            <option value="1" data-icon="check-circle">{{ __('Completed') }}</option>
                            <option value="2" data-icon="times-circle">{{ __('Cancel') }}</option>
                        </select>
                        <div class="status-preview mt-3">
                            <small class="text-muted">
                                <i class="las la-info-circle"></i>
                                {{ __('Changing the status will notify the client via email') }}
                            </small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="las la-save"></i> {{ __('Update Status') }}
                    </button>
                </div>
            </form>
        </div>
