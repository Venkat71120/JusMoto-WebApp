                                ...
                                <form action="{{route('admin.edit.product', $product->id)}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div  class="add-listing-content-wrapper mt-4">
                                        <div class="tab-content add-listing-content" id="add-listing-tabContent">
                                            <!-- product Info start-->
                                           @include('backend.pages.products.admin.edit-product-details')
                                            <!-- service Info end-->
                                            <!-- Include start-->
                                            @include('backend.pages.products.admin.edit-product-include')
                                            <!-- Include end-->
                                            <!-- Car Select start-->
                                            @include('backend.pages.products.admin.edit-product-car')
                                            <!-- Car select end-->
                                        </div>
                                    </div>
                                </form>
        ...
        (function ($) {
            ...
            $(document).ready(function () {
                ...
                $('#addEditAllVariant').on('click', function () 
                {
                    ...
                    $.ajax({
                                method: 'post',
                                url: "{{ route('admin.editCarProduct.add') }}",
                                data: {
                                    variant_id:'all',
                    ...
                    $.ajax({
                        ...
                        success: function(res) 
                        {
                            if (res.status == 'success') 
                            {
                                ...
                                $("#service_product_edit_img .thumbnail img").attr("src", "{{ asset('assets/frontend/img/gallery/single-image-upload.png') }}");
                                $('#addModal').modal('hide');
                                $('#isModalOpen').val('false');
                ...
                $('#edit_filterBtn').on('click', function () {
                    ...
                    $.ajax({
                        url: "{{ route('admin.editCarProduct.filter') }}",
                        method: 'GET',
