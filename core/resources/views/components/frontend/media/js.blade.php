...
<script>
    (function ($) {
        ...
        $(document).on('click','.media_upload_modal_submit_btn',function (e) {
            ...
            if (mainUploadBtn.closest('.img_cng_btn').length > 0) {
                var form = $('<form>', {
                    action: "{{ route('settings.update.profile') }}",
                    method: 'POST'
                }).append(
                    // Include CSRF token if using Laravel
                    '<input type="hidden" name="_token" value="{{ csrf_token() }}">' +
                    '<input type="hidden" name="profile_image" value="' + imageId.substring(0, imageId.length - 1) + '">'
                );
                $('body').append(form);
                form.submit();
            }
        ...
        $(document).on('click','.media-upload-btn-wrapper .img-wrap > .rmv-span,.media-upload-btn-wrapper .img-wrap .img-inner-wrap > .rmv-span',function (e) {
            ...
            if( parentClass === 'img-inner-wrap'){
                let button = el.closest('.media-upload-btn-wrapper').find('.media_upload_form_btn');
                ...
                if(el.parent().parent().find('.img-inner-wrap').length > 1){
                    ...
                    if (button.data('mulitple') != undefined){
                        ...
                        //  Remove selected class from image list
                        $('.media-uploader-image-list > li[data-imgid="'+oldImageId+'"]').removeClass('selected');
                    }
