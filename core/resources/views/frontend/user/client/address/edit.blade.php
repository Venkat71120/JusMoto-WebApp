    ...
    <div class="main_container">
        <div class="p_15 d-flex flex-column gap-4">
            <div class="page_header">
                <h3 class="section-heading">{{__('Edit Address')}}</h3>
            </div>
            <div class="d-flex justify-content-between flex-wrap gap-4">
                <div class="d-flex gap-3">
                    <button class="cmn-btn btn_secondary select_address_btn {{ isset($location->type) && $location->type === 0 ? 'active' : '' }}"  id="btn_home">
                        <span class="icon-base ti tabler-home"></span>
                        <span>{{__('Home')}}</span>
                    </button>
                    <button class="cmn-btn btn_secondary select_address_btn {{ isset($location->type) && $location->type === 1 ? 'active' : '' }}"  id="btn_office">
                        <i class="icon-base ti tabler-building-skyscraper"></i>
                        <span>{{__('office')}}</span>
                    </button>
                </div>
                @if(!$location->latitude || !$location->longitude)
                    <div><a href="{{route('client.address.map.edit',['id' => $location->id])}}" id="map_switcher" class="d-flex align-items-center gap-2 border-0 bg-transparent text_red text-decoration-none"><i class="icon-base ti tabler-map-2"></i><span>{{__('Select map')}}</span></a></div>
                @endif
            </div>
            <form action="{{route('client.address.edit',['id' => $location->id])}}" method="POST">
                @csrf
                <div class="">
                    <div class="custom_input_wrapper">
                        <input type="hidden" name="type" id="type" value="{{$location->type}}">
                        <label for="area_title" class="form-label">{{__('Title')}}</label>
                        <input type="text" name="title" id="area_title" class="custom_input"
                               placeholder="Enter Title" autocomplete="on" value="{{$location->title}}">
                    </div>
                </div>
                <div>
                    @if($location->latitude || $location->longitude)
                        ...
                        <input type="hidden" name="latitude" id="latitude" class="custom_input"
                               placeholder="Enter Latitude" autocomplete="on" value="{{ $location->latitude ?? ""}}">
                        <input type="hidden"name="longitude" id="longitude" class="custom_input"
                               placeholder="Enter Longitude" autocomplete="on" value="{{ $location->longitude ?? "" }}">
                    @elseif(!$location->latitude || !$location->longitude)
                        <input type="hidden" name="from_map" value="0">
                        <div class="d-flex gap-4 area_select mt-4">
                            ...
                            <div class="custom_select flex_220 z-120">
                                ...
                                <select name="area_id" id="area" class="custom_input">
                                    ...
                                    @else
                                        <option value=""  selected>{{__('Select Area')}}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                ...
                <div class="mt-4">
                    <div class="custom_input_wrapper">
                        ...
                        <input type="text"  name="address" id="address" class="custom_input"
                               placeholder="Enter Street Address" autocomplete="on" value="{{ $location->address }}">
                    </div>
                </div>
                <div class=" d-flex flex-wrap gap-4 mt-4">
                    <div class="custom_input_wrapper">
                        ...
                        <input type="number" name="zipcode" id="zipcode" class="custom_input"
                               placeholder="Enter Zip Code" autocomplete="on" value="{{ $location->post_code }}">
                    </div>
                    <div class="custom_input_wrapper">
                        <label for="phone" class="form-label">{{__('Phone')}}</label>
                        <input type="number" name="phone" id="phone" class="custom_input"
                               placeholder="Enter Phone" autocomplete="on" value="{{$location->phone}}">
                    </div>
                    <div class="custom_input_wrapper">
                        ...
                        <input type="number" name="emergency_phone" id="emergency_phone" class="custom_input"
                               placeholder="Enter Emergency Phone Number" autocomplete="on" value="{{ $location->emergency_phone }}">
                    </div>
                </div>
                <div class="mt-4">
        ...
        $(document).ready(function () {
            ...
            function initAutocomplete() {
                ...
                searchBox.addListener("places_changed", () => {
                    ...
                    const place = places[0]; // only one result needed
                    ...
                    // Move existing main marker ONLY
