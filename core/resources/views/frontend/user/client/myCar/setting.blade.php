<div id="popupContainer">
    ...
    <form action="{{ route('client.car.select') }}" method="POST" id="carSelectForm">
        @csrf
        ...
        <div class="popup containers shadow-lg rounded-2" id="stepOne">
            <div class="head position-relative">
                ...
                <h5 class="fs-md">{{ __('Select Your Brand') }}</h5>
            </div>
            <div class="model-box">
                ...
                <div class="models-grid" id="brandList">
                    <div class="loading text-center p-4">{{__('Loading brands...')}}</div>
                </div>
            </div>
            <div class="text-center mt-3">
                <button type="button" class="cmn-btn black-btn continue-btn">{{ __('Continue') }}</button>
            </div>
        ...
        <div class="popup containers shadow-lg rounded-2" id="stepTwo">
            <div class="head mb-3">
                <h1 class="title">{{__('Select Your Car')}}</h1>
            </div>
            <div class="model-box">
                ...
                <div class="models-grid" id="carList">
                    <div class="loading text-center p-4">{{__('Select brand first')}}</div>
                </div>
            </div>
            <div class="text-center mt-3">
                <button type="button" class="cmn-btn black-btn continue-btn">{{ __('Continue') }}</button>
        ...
        <div class="popup containers shadow-lg rounded-2" id="stepThree">
            ...
            <div class="model-car">
                ...
                <div class="show-car-text">
                    <h4 class="fs-md fw_semibold">{{ __('Car Transmission Type') }}</h4>
                    <div class="service-options" id="engineTypeList"></div>
                </div>

                <div class="show-car-text">
                    <h4>{{ __('Car Fuel Type') }}</h4>
                    <div class="d-flex align-items-center gap-4" id="fuelTypeList"></div>
                </div>
                <div class="show-car-text">
                    <h4 class="fs-md fw_semibold">{{ __('Registration Number') }}</h4>
                    <div class="form-group">
                        <input type="text" name="registration_number" id="registrationNumber" class="form-control"
                            placeholder="{{ __('Enter your car registration number') }}" maxlength="20">
                        <small class="text-muted">{{ __('e.g., TN01AB1234') }}</small>
                    </div>
                </div>
            </div>

            <div class="text-center mt-3">
                <button type="submit" class="cmn-btn black-btn">{{ __('Submit') }}</button>
            </div>
    ...
    <script>
        window.selectedCarFromBackend = {!! json_encode($selectedCar ?? null) !!};
