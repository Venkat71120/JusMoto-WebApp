@if($notification->type =='order')
    ...
    @if($order)
        <a class="notification_content {{($notification->is_read === 'unread') ? 'active' : ''}}" href="{{ route('order.details',  ['id' => $notification->identity, 'notificationId' => $notification->id]) }}">
...
@if($notification->type =='offer')
    ...
    @if($offer)
        <a class="notification_content {{($notification->is_read === 'unread') ? 'active' : ''}}" href="{{ route('offer.show',  ['offer_id' => $notification->identity, 'notificationId' => $notification->id]) }}">
            <span class="action-btn hw_40 bg_gray color_primary">
                <i class="icon-24px ti tabler-bell"></i>
            </span>
            <div>
...
@if($notification->type =='ticket' || $notification->type =='ticket-update')
    ...
    @if($ticket)
        <a class="notification_content {{($notification->is_read === 'unread') ? 'active' : ''}}" href="{{ route('ticket.details',  ['id' => $notification->identity, 'notificationId' => $notification->id]) }}">
            <span class="action-btn hw_40 bg_gray color_primary">
                <i class="icon-24px ti tabler-bell"></i>
            </span>
            <div>
