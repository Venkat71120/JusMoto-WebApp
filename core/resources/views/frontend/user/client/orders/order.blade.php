...
@section('content')
    ...
    <div class="main_container">
        <h4 class="section-header px_15 pt_15">{{__('Order List')}}</h4>
        ...
        <div class="mb-3">
            <form action="{{ route('orders.index') }}" method="GET" class="row g-3">
                ...
                <div class="col-md-3">
                    <button type="submit" class="btn_primary btn-sm py-2 px-3">{{__('Apply Filter')}}</button>
                    <a href="{{ route('orders.index') }}" class="btn btn-secondary">{{__('Reset')}}</a>
                </div>
            </form>
        </div>
        ...
        <div class="table_wrapper">
                    ...
                    <table class="data-table table w-100 br_4 overflow-hidden">
                        ...
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td>
                                <span class="table_payment {{ $order->payment_status == 1 ? 'complete' : 'pending' }}">
            ...
            @else
                <div class="alert alert-info text-center p-4">
                    ...
                    <p class="mb-0">
                        @if(request()->has('status') || request()->has('payment_status') || request()->has('search'))
                          {{__('No orders match your filter criteria. Try adjusting your filters.')}}
                        @else
                            {{__("You haven't placed any orders yet. Start ordering to see your order history!")}}
                        @endif
                    </p>
                    @if(request()->has('status') || request()->has('payment_status') || request()->has('search'))
                        <a href="{{ route('orders.index') }}" class="btn btn-primary mt-3">{{__('Clear Filters')}}</a>
                    @endif
                </div>
            @endif
        </div>
